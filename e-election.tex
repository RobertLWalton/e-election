...
\item A long truely random bit string, for use only if the
...
Voting Machine sofware versions known to the Registration Machines.
...
\item Public keys and UIDs of all Registration Machines.
\item The unmarked ballot.
\item UIDs and associated public keys of all Private Voting
Machines.  This includes de-registered and test Private Voting
...
During after-voting processing the following are added to each
Collection Machine Locked box:

\begin{enumerate}
\setcounter{enumi}{\value{CML-COUNTER}}
\item The UIDs and public keys of all other Collection Machines.
\item The UIDs and public keys of all Counting Machines.
\item For each count:
\begin{enumerate}
\item Set of UIDs of valid Private Voting Machines.
\item Sorted set of valid ballots.
\item Sorted set of valid ballot headers.
\item Sorted set of valid encrypted ballots.
\end{enumerate}
\end{enumerate}
...
Counting Machines recieve a complete sorted set of encrypted ballots
...
and a log of all
the vote counts.  It does not contain encrypted ballots, but does
contain within the log cryptographic hashes of encrypted ballot sets.
...
official count and the test re-count.
