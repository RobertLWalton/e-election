...
\item Public keys of all Registration Machines and all Collection
Machines.
\item The unmarked ballot.
\item UIDs and associated public keys of all Private Voting
Machines.  This includes de-registered and test Private Voting
...
the signatures of accepted submitted ballots
...
and a set of encrypted ballots.  The headers of a submitted ballot is
...
The Counting Machine locked box contains nothing but the names
and public keys of all Collection Machines.

An M-box contains nothing but the number (1 though 10)
...
As long as a Private Voting Machine locked box contains almost completely
correct names and public keys of Registration Machines,
it should be effectively impossible to suppress the
Private Voting Machine's vote.  This information is initially delivered
by the voter registration office of the town registering the voter, when
the office gives the voter the Private Voting Machine.  The information
can then be updated by having Registration Machines for a current
election distribute this information for future elections as part of
their response to a Request for Ballot.
...
The software on a thumb drive can contain names and public keys of
servers that have updates for the software, so the software can
update itself securely.  Of course, before updating the software
should check multiple servers to be sure they all agree on the
update.
